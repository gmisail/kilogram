...
trait Scanner {
    ...
    fn scan_conditional(&mut self, options: &[(char, TokenKind)], fallback: TokenKind) -> Token;
...
impl Scanner for ScannerContext {
    ...
    fn scan_conditional(&mut self, options: &[(char, TokenKind)], fallback: TokenKind) -> Token {
        for (c, kind) in options.iter() {
            if !self.is_at_end() && *c == self.peek_char().unwrap_or('\0') {
                ...
                return self.new_symbol_token(kind.clone());
            }
        }

        // TODO: actually handle lengths for multi-character symbols
        self.new_symbol_token(fallback)
    }
    ...
    fn scan_number(&mut self) -> Token {
        ...
        if self.match_char('.') {
            ...
            // TODO: handle errors here, i.e. there is no integer after the '.' token
            ...
            let fraction = match fractional_comp.kind {
                TokenKind::Integer(num) => num,
                _ => 0,
            };
        ...
        if self.is_at_end() {
            ...
            match self.next_char() {
                Some(c) => match c {
                    ...
                    ch if ch.is_numeric() => Ok(self.scan_number()),
                    ch if ch.is_alphabetic() || ch == '_' => Ok(self.scan_identifier()),
                    '"' => Ok(self.scan_string()),
                    ...
                    '!' => {
                        Ok(self.scan_conditional(&[('=', TokenKind::NotEqual)], TokenKind::Bang))
                    }
                    '=' => {
                        Ok(self.scan_conditional(&[('=', TokenKind::Equality)], TokenKind::Equal))
                    }
                    '>' => {
                        Ok(self
                            .scan_conditional(&[('=', TokenKind::GreaterEq)], TokenKind::Greater))
                    }
                    '<' => Ok(self.scan_conditional(&[('=', TokenKind::LessEq)], TokenKind::Less)),
